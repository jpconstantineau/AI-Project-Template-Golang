...
## 3) SQLite: No CGO + Versioned Migrations (Required)

### 3.1 Driver
- Use a pure-Go SQLite driver (recommendation: `modernc.org/sqlite`).
- Ensure builds pass with `CGO_ENABLED=0`.

### 3.2 DB Location and Configuration
...
- **Preferred**: `PRAGMA user_version` as the schema version store.
...
## 5) Clean Code + Idiomatic Go (Mandatory)
...
## 6) Design Patterns (When Appropriate) + Testing Requirements
...
## 9) PowerShell Build Scripts (Required)
...
- `-Task run` → runs `cmd/game` with optional args
...
## 10) README Sync Rules (Non-Negotiable)
...
## 12) What to Do When Implementing a New Feature (Checklist)
